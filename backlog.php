...
<html>
   <head>
      ...
      <title>Maintenance Backlog</title>
      ...
      <?php include 'sideNav.php' ?>
         ...
         <div class="d-flex flex-column" id="content-wrapper">
            ...
            <?php
               ...
               $get_pro = "SELECT asset_id as 'Asset ID', serviceDue as 'Service Due', assetType, manufacturer, DATEDIFF(NOW(), serviceDue) as 'Delay' from NonCurrentAsset where DATEDIFF(NOW(), serviceDue) > 0 and DATEDIFF(NOW(), serviceDue) <= 90;";
               
               $run_pro = mysqli_query($Con,$get_pro);
               echo "<script>";
               echo "var records = [";
               while($row_pro = mysqli_fetch_array($run_pro)) {
                  $asset_id = $row_pro['Asset ID'];
                  $serviceDue = $row_pro['Service Due'];
                  $assetType = $row_pro['assetType'];                  
                  $manufacturer = $row_pro['manufacturer'];  
                  $delay = $row_pro['Delay'];                
                  
                  echo "{'asset_id': '$asset_id', 'asset_type': '$assetType', 'serviceDue': '$serviceDue', 'manufacturer': '$manufacturer', 'delay': '$delay'},";
            ...
            <div class="container-fluid">
                <div class="card shadow">
                    <div class="card-header py-3">
                        <p class="text-primary m-0 fw-bold">Backlog</p>
                    </div>
                    ...
                    <canvas style=" height: 300px;" id="myChart" style="width:100%;max-width:1000px"></canvas>
                        <script>
                            ...
                            options: {
                                ...
                                title: {
                                display: true,
                                text: "Days Passed by Asset Code"
                                }
                            }
                            });
                        </script>
            ...
            <br>
               <div class="container-fluid">
                  <h3 class="text-dark mb-4">Backlog</h3>
                  <div class="card shadow">
                     <div class="card-header py-3">
                        <p class="text-primary m-0 fw-bold">Backlog Information</p>
                     </div>
                     <div class="card-body">
                        <div class="row">
   ...
   buildTable(records);
    ...
    function filterData(value){
        ...
        for (let i = 0; i < records.length; i++) {
            ...
            var manu = records[i].manufacturer.toLowerCase();
    ...
    function buildTable(records){
        ...
        for (let i = 0; i < records.length; i++) {
            var row = `<tr>
                        ...
                        <td> ${records[i].manufacturer} </td>
                        <td> ${records[i].serviceDue} </td>
                        <td> ${records[i].delay} </td>                        
                        </tr>`;
